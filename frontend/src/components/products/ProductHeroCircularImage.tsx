...
export default function ProductHeroCircularImage({ product: p }: { product: ProductDetail }) {
  const badges = p.hero_badges?.length > 0 
    ? p.hero_badges 
    : ["Trusted Platform", "Patient-Centric", "Better Health Outcomes"];

  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center py-2">
      {/* Left Column: Eyebrow, Title, Subtitle, and Inline Red Icon Badges */}
      <div className="max-w-2xl">
        ...
        {/* Bottom Red Pill/Circle Badges Row */}
      ...
      {/* Right Column: Circular Image Mask & Tagline with Bottom Red Accent Bar */}
      <div className="flex items-center justify-center lg:justify-end gap-6">
        <div className="relative h-48 w-48 md:h-56 md:w-56 flex-shrink-0 rounded-full p-1 bg-white/10 shadow-2xl overflow-hidden">
          {p.hero_image ? (
            <img src={p.hero_image} alt={p.name} className="h-full w-full rounded-full object-cover" />
          ) : (
            <img 
        ...
        <div className="max-w-[150px]">
          <p className="text-sm md:text-base font-semibold leading-snug text-white">
            {p.hero_tagline || "Your Health In Your Hands Anytime Anywhere"}
          </p>
